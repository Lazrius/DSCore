...
/** @type {Uint32Array} Freelancer CRC32 hash table */
...
/** @type {Map} Reverse dictionary (Hash->String) */
const dictionary = new Map();

/**
 * FLcrc32 hash function implementation
 * @param  {String}  value         Text to hash
 * @param  {Boolean} caseSensitive Case sensitive
 * @param  {Boolean} signed        Signed/unsigned return number
 * @return {Number}
 */
export function flcrc32(value, caseSensitive = false, signed = false) {
	if (value === null) return 0;
	if (Number.isInteger(value)) return value;
	if (typeof value != "string") throw new TypeError("Invalid hash value type");
	if (! caseSensitive) value = value.toLowerCase();

	let crc = 0xFFFFFFFF;

	for (let code, i = 0; (i < value.length) && (code = value.charCodeAt(i)); i++)
		if (code > 0xFF) throw new RangeError("Unicode chars are not permitted");
			else crc = (crc >>> 8) ^ (hashTable[(crc & 0xFF) ^ code] >>> 0);

	crc = signed ? ~crc << 0 : ~crc >>> 0;
	// dictionary.set(crc, value); // For debugging purposes reverse dictionary is built 
	return crc;
}